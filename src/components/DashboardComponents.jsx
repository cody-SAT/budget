...
export const KpiCard = ({ title, value, change, isPositive, isCurrency = true }) => (
  <Card>
...
export const SankeyTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white p-3 border rounded shadow-lg z-50">
        <p className="label font-bold text-gray-700">{name}</p>
        <p className="text-indigo-600">{formatLargeNumber(value)}</p>
      </div>
    );
  }
  return null;
        ...
        node={({ x, y, width, height, index, payload, containerWidth }) => {
          ...
          return (
            <g transform={`translate(${x},${y})`}>
              <rect width={width} height={height} fill={payload.color} />
              <text
