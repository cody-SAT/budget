...
import {
  ...
  CardHeader,
  CardTitle,
} from './ui/card'; // FIX: Relative path

import { SEGMENT_COLORS, OPEX_COLORS } from '../data/mockData'; // FIX: Relative path
...
export const RevenueToProfitSankey = ({ data, year, filter }) => {
  const sankeyData = useMemo(() => {
    ...
    let totalRevenue = 0, totalCostOfRevenue = 0, totalTac = 0, totalOpexRD = 0, totalOpexSM = 0, corporateCosts = 0;
    const segmentRevenues = { 'Google Services': 0, 'Google Cloud': 0, 'Other Bets': 0 };

    yearData.forEach(item => {
      if (item.segment === 'Google Services' && filter['Google Services']) segmentRevenues['Google Services'] += item.revenue;
      else if (item.segment === 'Google Cloud' && filter['Google Cloud']) segmentRevenues['Google Cloud'] += item.revenue;
      else if (item.segment === 'Other Bets' && filter['Other Bets']) segmentRevenues['Other Bets'] += item.revenue;
      ...
      if (item.segment === 'Corporate Costs') corporateCosts = Math.abs(item.operatingIncome) || 0;
    });
    ...
    const totalOpEx = totalOpexRD + totalOpexSM;
  ...
  if (!sankeyData || sankeyData.nodes.length === 0 || sankeyData.links.length === 0) {
    return <ChartWrapper title="Revenue to Profit Flow" height="h-[600px]"><div className="flex items-center justify-center h-full text-gray-500">Select segments to display data.</div></ChartWrapper>;
  }
  ...
  const SankeyNode = ({ x, y, width, height, index, payload, containerWidth }) => {
    ...
    return (
      <g transform={`translate(${x},${y})`}>
        ...
        <text x={isSource ? width + 6 : -6} y={height / 2} textAnchor={isSource ? 'start' : 'end'} dominantBaseline="middle" fill="#333" fontSize="12" fontWeight="500">
          {node.name}
        </text>
      </g>
    );
  ...
  return (
    <ChartWrapper title={`Revenue to Operating Profit Flow (${year})`} height="h-[600px]">
      <Sankey data={sankeyData} nodePadding={50} margin={{ top: 20, right: 100, bottom: 20, left: 100 }} link={{ stroke: '#B0B0B0', strokeOpacity: 0.3 }} node={<SankeyNode />}>
