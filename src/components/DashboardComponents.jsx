...
import {
  ...
  formatPercent,
  formatNumber,
} from '../lib/utils.js'; // FIX: Relative path
...
import {
  ...
  CardHeader,
  CardTitle,
} from './ui/card.jsx'; // FIX: Relative path

import { SEGMENT_COLORS, OPEX_COLORS } from '../data/mockData.js'; // FIX: Relative path
...
export const SankeyTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    ...
    // --- THIS IS THE FIX ---
    // We add a "guard clause" to check if the data exists before accessing it.
    // This prevents the "Cannot read properties of undefined" crash.
    if (!payload[0].payload || !payload[0].payload.source || !payload[0].payload.target) {
      return null; // Don't render if data is malformed
    }
    // --- END OF FIX ---
